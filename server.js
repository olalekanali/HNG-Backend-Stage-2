...
// Ensure columns allow NULL (safe migration)
(async () => {
  try {
...
// Root
app.get("/", (req, res) =>
  res.json({ message: "Welcome to the Country REST API 🚀" })
);

// ------------------------------------------------------------
// POST /countries/refresh — Faster, asynchronous post-tasks
// ------------------------------------------------------------
app.post("/countries/refresh", async (req, res) => {
  ...
  try {
    ...
    const [countriesResp, exchangeResp] = await Promise.all([
      axiosInstance.get(countriesUrl),
      axiosInstance.get(exchangeUrl),
    ]);
    ...
    if (!exchangeData?.rates || exchangeData.result !== "success")
    ...
    try {
      ...
      res
        .status(200)
        .json({ message: "Countries refreshed successfully", total: processed });

      // ⚙️ Run heavy post-processing asynchronously
      process.nextTick(async () => {
        try {
          // Cache data for GET /countries
          const [rows] = await database.query("SELECT * FROM countries");
          if (!fs.existsSync(CACHE_DIR)) fs.mkdirSync(CACHE_DIR, { recursive: true });
          fs.writeFileSync(
            path.join(CACHE_DIR, "countries.json"),
            JSON.stringify(rows, null, 2)
          );

          await generateSummaryImage();
          console.log("Post-refresh tasks completed.");
        } catch (err) {
...
// ------------------------------------------------------------
// GET /countries — optional filters and sorting
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// GET /countries/image — serves summary image
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// GET /countries/:name — fetch single country
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// Helper: generateSummaryImage (async post-task)
// ------------------------------------------------------------
async function generateSummaryImage() {
  try {
    ...
    rows.forEach((r, i) => {
      const gdpStr = Number(r.estimated_gdp).toLocaleString(undefined, {
    ...
    console.log("Summary image updated:", SUMMARY_IMAGE);
  } catch (err) {
